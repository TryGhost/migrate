...
export function createProductImporter({
    oldStripe,
    newStripe,
    reporter
}: {
    ...
    const provider = {
        ...
        async findExisting(oldItem: Stripe.Product) {
            ...
            if (!cachedProducts) {
                await reuse.schedule('findExisting', async () => {
                    ...
                    for await (const product of newStripe.useAsyncIterator(client =>
        ...
        async recreate(oldProduct: Stripe.Product) {
            return await ifDryRunJustReturnFakeId(async () => {
                const product = await newStripe.use(client =>
        ...
        async revert(_: Stripe.Product, newProduct: Stripe.Product) {
            ...
            const prices = await newStripe.use(client =>
                client.prices.list({
            ...
            return await ifNotDryRun(async () => {
                ...
                if (prices.data.length === 0) {
                    ...
                    await newStripe.use(client =>
                        client.products.update(getObjectId(newProduct), {
                            active: false
                        })
                    );
                }

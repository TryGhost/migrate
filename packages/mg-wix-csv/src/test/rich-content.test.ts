...
describe('Wix rich content converter', () => {
    ...
    it('converts common block and inline nodes', () => {
            ...
            nodes: [
                {
                    ...
                    headingData: {level: 2},
                    nodes: [
                        {type: 'TEXT', textData: {text: 'Heading', decorations: [{type: 'BOLD'}, {type: 'ITALIC'}]}}
                    ]
                },
                {
                    type: 'PARAGRAPH',
                    nodes: [
                        ...
                        {type: 'TEXT', textData: {text: ' super', decorations: [{type: 'SUPERSCRIPT'}]}},
                        {
                            ...
                            textData: {
                                text: ' link',
                                decorations: [{type: 'LINK', linkData: {link: {url: 'https://example.com?a=1&b=2'}}}]
                            }
                        },
                    ...
                    nodes: [
                        {
                            ...
                            nodes: [
                                {
                                    type: 'PARAGRAPH',
                                    nodes: [{type: 'TEXT', textData: {text: 'Bullet', decorations: []}}]
                                }
                            ]
                        }
                    ]
                    ...
                    nodes: [
                        {
                            type: 'LIST_ITEM',
                            nodes: [
                                {type: 'PARAGRAPH', nodes: [{type: 'TEXT', textData: {text: 'One', decorations: []}}]}
                            ]
                        }
                    ]
                },
        ...
        assert.equal(
            ...
            '<h2><em>Heading</em></h2><p><strong>Bold</strong> <em>italic</em> <u>underline</u> <sup>super</sup> <a href="https://example.com?a=1&amp;b=2">link</a> color</p><ul><li><p>Bullet</p></li></ul><ol><li><p>One</p></li></ol><hr>'
        );
        assert.equal(renderNode({type: 'HEADING', headingData: {level: 2}}), '<h2></h2>');
        assert.equal(
            renderNode({type: 'HEADING', headingData: {level: 2}, nodes: [{type: 'UNKNOWN'}]}),
        ...
        assert.equal(
            renderNode({type: 'PARAGRAPH', nodes: [{type: 'TEXT', textData: {text: '   ', decorations: []}}]}),
            ''
        );
        ...
        assert.equal(
            renderNode({
                type: 'TEXT',
                textData: {text: ' It ', decorations: [{type: 'LINK', linkData: {link: {url: 'https://example.com'}}}]}
            }),
            ' <a href="https://example.com">It</a> '
        );
        assert.equal(
            renderNode({
                type: 'TEXT',
                textData: {
                    text: 'Bad link',
                    decorations: [{type: 'LINK', linkData: {link: {url: 'data:text/html,unsafe'}}}]
                }
            }),
            'Bad link'
        );
    ...
    it('converts images, buttons, and tables', () => {
        ...
        assert.equal(
            renderNode({
                type: 'BUTTON',
                buttonData: {text: 'Go', link: {url: 'https://example.com'}, containerData: {alignment: 'CENTER'}}
            }),
            '<div class="kg-card kg-button-card kg-align-center"><a href="https://example.com" class="kg-btn kg-btn-accent">Go</a></div>'
        );
        assert.equal(
            renderNode({
                type: 'BUTTON',
                buttonData: {text: 'Left', link: {url: 'https://example.com'}, containerData: {alignment: 'LEFT'}}
            }),
            '<div class="kg-card kg-button-card kg-align-left"><a href="https://example.com" class="kg-btn kg-btn-accent">Left</a></div>'
        );
        assert.equal(
            renderNode({
                type: 'BUTTON',
                buttonData: {
                    ...
                    containerData: {alignment: 'AUTO'}
                }
            }),
            '<div class="kg-card kg-button-card kg-align-center"><a href="https://example.com" class="kg-btn kg-btn-accent">Bad alignment</a></div>'
        );
        assert.equal(
            renderNode({type: 'BUTTON', buttonData: {text: 'Unsafe', link: {url: 'javascript:alert(1)'}}}),
            '<p>Unsafe</p>'
        );
                    ...
                    nodes: [
                        {
                            type: 'TABLE_CELL',
                            nodes: [
                                {type: 'PARAGRAPH', nodes: [{type: 'TEXT', textData: {text: 'Cell', decorations: []}}]}
                            ]
                        }
                    ]
                }
    ...
    it('uses HTML cards for unknown or invalid content', () => {
        assert.equal(
            renderNode({type: 'IMAGE'}),
            '<!--kg-card-begin: html-->\n<div class="mg-html-card">\n<pre>{\n  &quot;type&quot;: &quot;IMAGE&quot;\n}</pre>\n</div>\n<!--kg-card-end: html-->'
        );
        assert.equal(
            renderNode({type: 'UNKNOWN', nodes: [{type: 'TEXT', textData: {text: 'child', decorations: []}}]}),
            '<!--kg-card-begin: html-->\n<div class="mg-html-card">\nchild\n</div>\n<!--kg-card-end: html-->'
        );
        assert.match(renderNode({type: 'UNKNOWN'}), /&quot;UNKNOWN&quot;/);
        assert.equal(
            richContentToHtml({richContent: '', plainContent: 'Fallback'}),
            '<!--kg-card-begin: html-->\n<div class="mg-html-card">\n<p>Fallback</p>\n</div>\n<!--kg-card-end: html-->'
        );
        assert.equal(
            richContentToHtml({richContent: '{"nodes":[]}', plainContent: 'Fallback'}),
            '<!--kg-card-begin: html-->\n<div class="mg-html-card">\n<p>Fallback</p>\n</div>\n<!--kg-card-end: html-->'
        );
        assert.equal(
            richContentToHtml({richContent: '{"nodes":{}}', plainContent: 'Fallback'}),
        ...
        assert.equal(
            richContentToHtml({richContent: 'bad json', plainContent: 'Fallback'}),
            '<!--kg-card-begin: html-->\n<div class="mg-html-card">\n<p>Fallback</p>\n</div>\n<!--kg-card-end: html-->'
        );
    });
});
